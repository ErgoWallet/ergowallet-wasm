...
use sigma_tree::chain::{Input, Base16EncodedBytes, Contract};
...
impl Address {
    ...
    pub fn from_public_key(pub_key: &[u8]) -> Address {
        ...
        let p2pk_address: chain::Address = chain::Address::P2PK(
            sigma_protocol::ProveDlog::sigma_parse_bytes(content_bytes).unwrap()
        );
...
impl Transaction {
    ...
    pub fn serialize(js_tx: &JsValue) -> Vec<u8> {
    ...
    pub fn create(
        ...
        let _inputs: Vec<chain::Input> = inputs_from_js.iter().map(|x| {
        ...
        // construct outputs
        let mut _outputs: Vec<chain::ErgoBoxCandidate> = outputs_from_js.iter().map(|x| {
